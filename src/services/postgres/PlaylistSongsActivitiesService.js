...
class PlaylistSongsActivitiesService {
    ...
    async getPlaylistSongActivities(playlistId) {
        const query = {
            text: `SELECT B.username, C.title, A.action, A.time FROM playlist_songs_activities A LEFT JOIN users B ON B.id = A.user_id LEFT JOIN songs C ON C.id = A.song_id WHERE A.playlist_id = $1 ORDER BY A.action`,
            values: [playlistId],
        };
